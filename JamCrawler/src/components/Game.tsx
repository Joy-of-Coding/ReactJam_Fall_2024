...
import CombatStageModal from "./Combat/CombatStageModal";
...
export default function Game() {
    ...
    const movePlayer = (dx: number, dy: number) => {
        ...
        //console.log("outside state:", combatNewPos);
        setPlayer((prev) => {
            const newPos = { x: prev.position.x + dx, y: prev.position.y + dy };
            //console.log("inside state:", newPos);
            if (
                newPos.x < 0 ||
                newPos.x >= GRID_SIZE ||
                newPos.y < 0 ||
        ...
        if (
            ...
            // set inCombat to true (trigger for combat modal pop up)
            setInCombat(true);
            // from inside CombatStageModal we check if inCombat is true and then the user
            // can click a button to actually start the combat.
    ...
    return (
        <>
            ...
            <CombatStageModal inCombat={inCombat} />
